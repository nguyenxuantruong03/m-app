...
const MentionPlugin = () => {
  ...
  const fetchEmails = async (): Promise<string[]> => {
    try {
      ...
      return response.data.map((staff: any) => staff.email); // Giả sử email được lấy từ trường 'email' của dữ liệu
    } catch (error) {
  ...
  const mentionPlugin = {
    ...
    render: () => {
      ...
      } as Record<string, any>; // Explicitly define the return type
    },
  };
