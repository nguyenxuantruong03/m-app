...
} from "@/actions/client/products/get-product-search";
import { ProductUnion } from "@/hooks/client/use-cart";
    ...
    Promise.all(productNamesPromises)
      .then((productNames) => {
        const results = productNames
          .flat()
          .map((product: ProductUnion | Product) => ({
            product,
          ...
          .map(({ product }) => ({
            ...
            //TODO
            // imageUrl: product.images[0].url ? product.images[0].url : null,
            name: product.name,
