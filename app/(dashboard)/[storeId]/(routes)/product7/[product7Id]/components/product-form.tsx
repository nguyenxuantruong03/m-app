...
import { Category7, Product7, Image7, Size, Color, Specifications,Salientfeatures } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/product7/${params.product7Id}`, data);
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/product7/${params.product7Id}`);
      router.refresh();
      router.push(`/${params.storeId}/product7`);
