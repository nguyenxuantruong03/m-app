...
import { getEnterNameTranslation, getProductClient, translateProductsClientSocket } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientSocketMessage = translateProductsClientSocket(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product7`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientSocketMessage.productSocket} (${data.length})`}
          description={productClientSocketMessage.manageProductSocket}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product7/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product7Id" entityName="product7" />
    </>
  );
};
