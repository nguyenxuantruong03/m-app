...
const formSchema = z.object({
  ...
  imagesalientfeaturesproduct11: z.object({ url: z.string() }).array(),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if (initialData) {
        await axios.patch(
          `/api/${params.storeId}/product11/${params.product11Id}`,
          data
        );
      } else {
        await axios.post(`/api/${params.storeId}/product11`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/product11`);
    ...
    <>
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormField
            control={form.control}
            name="imagesalientfeaturesproduct11"
            render={({ field }) => (
              <FormItem>
                <FormLabel>Images salientfeatures product 9(Please add only 2 images)</FormLabel>
                <FormControl>
                  <ImageUpload
                    value={field.value.map((image) => image.url)}
                    disabled={loading}
