...
import { Category, Product, Image, Size, Color, Specifications,Salientfeatures, Ipad, ImageIpad, Headphone, ImageHeadphone } from "@prisma/client"
...
const formSchema = z.object({
  ...
  imagesheadphone: z.object({url: z.string()}).array(),
...
interface ProductFormProps {
  initialData: Headphone & {
    imagesheadphone: ImageHeadphone[]} | null;
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const title = initialData ? 'Edit headphones' : 'Create headphones';
  const description = initialData ? 'Edit a headphones.' : 'Add a new headphones';
  const toastMessage = initialData ? 'Headphones updated.' : 'Headphones created.';
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/headphone/${params.headphoneId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/headphone`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/headphone`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/headphone/${params.headphoneId}`);
      router.refresh();
      router.push(`/${params.storeId}/headphone`);
      toast.success('Headphones deleted.');
    } catch (error: any) {
      toast.error('Make sure you removed all headphones using this product first.');
    } finally {
      setLoading(false);
      setOpen(false);
    }
