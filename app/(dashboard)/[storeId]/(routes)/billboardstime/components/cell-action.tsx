...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onReset = async () => {
    try {
      setLoading(true);
      const promise = axios.post(`/api/${params.storeId}/billboardstime/${data.id}`);
      await toast.promise(
        promise.then(() => {
          return (
            <p>
              Làm mới thành công! Thời gian: <span className="font-bold">{data.timeout}</span>
            </p>
          );
        }),
        {
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/billboardstime/${data.id}`)
            }
          >
