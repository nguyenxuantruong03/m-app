...
export const CategoryForm: React.FC<CategoryFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
      ...
      if (initialData) {
        promise = axios.patch(
          `/api/${params.storeId}/categories6/${params.category6Id}`,
          data
        );
      } else {
        promise = axios.post(`/api/${params.storeId}/categories6`, data);
      }
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex space-x-3 items-center">
                    ...
                    <Recommend message="Hãy đặt tên phù hợp với từng loại hàng. VD: Keo con chó, Keo dán sắt..." />
                  </FormLabel>
                  <FormControl>
                    <Input
                      disabled={loading}
