...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientGlue } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const categoriesClientGlueMessage = translateCategoriesClientGlue(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories6`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${categoriesClientGlueMessage.categoryGlue} (${data.length})`}
          description={categoriesClientGlueMessage.manageCategoryGlue}
        />
        <div className=" flex space-x-3">
          <Downloadfile
            data={data}
            filename="categories6"
            languageToUse={languageToUse}
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/categories6/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="category6Id" entityName="categories6" />
    </>
  );
};
