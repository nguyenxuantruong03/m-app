...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const title = initialData ? "Edit ipad" : "Create ipad";
  const description = initialData ? "Edit a ipad." : "Add a new ipad";
  const toastMessage = initialData ? "Ipad updated." : "Ipad created.";
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh();
      router.push(`/${params.storeId}/ipad`);
      toast.success("Ipad deleted.");
    } catch (error: any) {
      toast.error("Make sure you removed all ipad using this product first.");
    } finally {
      setLoading(false);
      setOpen(false);
    }
