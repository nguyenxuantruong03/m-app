...
export const CategoryForm: React.FC<CategoryFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
      ...
      if (initialData) {
        promise = axios.patch(
          `/api/${params.storeId}/categories4/${params.category4Id}`,
          data
        );
      } else {
        promise = axios.post(`/api/${params.storeId}/categories4`, data);
      }
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex space-x-3 items-center">
                    ...
                    <Recommend message="Hãy đặt tên phù hợp với từng loại hàng. VD: Đá cắt Hải Dương..." />
                  </FormLabel>
                  <FormControl>
                    <Input
                      disabled={loading}
