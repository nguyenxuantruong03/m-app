...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientCuttingStone } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const categoriesClientCuttingStoneMessage = translateCategoriesClientCuttingStone(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories4`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${categoriesClientCuttingStoneMessage.categoryCuttingStone} (${data.length})`}
          description={categoriesClientCuttingStoneMessage.manageCategoryCuttingStone}
        />
        <div className=" flex space-x-3">
          <Downloadfile
            data={data}
            filename="categories4"
            languageToUse={languageToUse}
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/categories4/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="category4Id" entityName="categories4" />
    </>
  );
};
