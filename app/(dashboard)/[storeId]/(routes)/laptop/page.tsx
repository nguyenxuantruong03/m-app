...
const ProductPage = async ({ params }: { params: { storeId: string } }) => {
  const laptops = await prismadb.laptop.findMany({
    where: {
  ...
  const formattedProduct: ProductColumn[] = laptops.map((item) => ({
    id: item.id,
    name: item.name,
    ...
    guaranteedescription: formatter.format(item.guaranteedescription.toNumber()),
    ...
    category: item.categorylaptop.name,
