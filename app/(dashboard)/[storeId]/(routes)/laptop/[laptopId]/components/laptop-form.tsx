...
import { Category, Product, Image, Size, Color, Specifications,Salientfeatures, Ipad, ImageIpad, Laptop, ImageLaptop } from "@prisma/client"
...
const formSchema = z.object({
  ...
  imageslaptop: z.object({url: z.string()}).array(),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const title = initialData ? 'Edit laptop' : 'Create laptop';
  const description = initialData ? 'Edit a laptop.' : 'Add a new laptop';
  const toastMessage = initialData ? 'Laptop updated.' : 'Laptop created.';
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/laptop/${params.laptopId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/laptop`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/laptop`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/laptop/${params.laptopId}`);
      router.refresh();
      router.push(`/${params.storeId}/laptop`);
