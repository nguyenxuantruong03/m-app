...
import { getEnterNameTranslation, getProductClient, translateProductsClientGlue } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientGlueMessage = translateProductsClientGlue(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product6`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientGlueMessage.productGlue} (${data.length})`}
          description={productClientGlueMessage.manageProductGlue}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product6/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product6Id" entityName="product6" />
    </>
  );
};
