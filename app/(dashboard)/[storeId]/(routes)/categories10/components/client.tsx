...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientLightbulb } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const showAPIRole = isRole;
    ...
    const categoriesClientLightbulbMessage = translateCategoriesClientLightbulb(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories10`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      ...
      <Heading
          title={`${categoriesClientLightbulbMessage.categoryLightbulb} (${data.length})`}
          description={categoriesClientLightbulbMessage.manageCategoryLightbulb}
        />
        <div className=" flex space-x-3">
          <Downloadfile data={data} filename="categories10" languageToUse={languageToUse}/>
          <Button
            onClick={() => router.push(`/${params.storeId}/categories10/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="category10Id" entityName="categories10" />
    </>
  );
};
