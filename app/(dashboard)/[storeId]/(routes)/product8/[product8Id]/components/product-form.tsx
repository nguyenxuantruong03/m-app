...
const formSchema = z.object({
  ...
  imagesalientfeaturesproduct8: z.object({ url: z.string() }).array(),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if (initialData) {
        await axios.patch(
          `/api/${params.storeId}/product8/${params.product8Id}`,
          data
        );
      } else {
    ...
    <>
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormField
            control={form.control}
            name="imagesalientfeaturesproduct8"
            render={({ field }) => (
              <FormItem>
                <FormLabel>Images Salientfeatures product 8(Please add only 2 images)</FormLabel>
                <FormControl>
                  <ImageUpload
                    value={field.value.map((image) => image.url)}
                    disabled={loading}
