...
import { getEnterNameTranslation, getProductClient, translateProductsClientPaint } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientPaintMessage = translateProductsClientPaint(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product8`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientPaintMessage.productPaint} (${data.length})`}
          description={productClientPaintMessage.manageProductPaint}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product8/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product8Id" entityName="product8" />
    </>
  );
};
