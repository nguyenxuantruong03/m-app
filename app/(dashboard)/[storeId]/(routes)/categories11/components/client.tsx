...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientCommonGoods } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories11`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <div className=" flex space-x-3">
          <Downloadfile
            data={data}
            filename="categories11"
            languageToUse={languageToUse}
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/categories11/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="category11Id" entityName="categories11" />
    </>
  );
};
