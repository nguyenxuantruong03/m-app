...
export const CategoryForm: React.FC<CategoryFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
      ...
      if (initialData) {
        promise = axios.patch(
          `/api/${params.storeId}/categories11/${params.category11Id}`,
          data
        );
      } else {
        promise = axios.post(`/api/${params.storeId}/categories11`, data);
      }
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex space-x-3 items-center">
                    ...
                    <Recommend message="Hãy đặt tên phù hợp với từng loại hàng. VD: Kéo, dao rọc giấy..." />
                  </FormLabel>
                  <FormControl>
                    <Input
                      disabled={loading}
