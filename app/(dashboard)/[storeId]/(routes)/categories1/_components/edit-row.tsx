...
const EditRow: React.FC<EditRowProps> = ({ data, id, language }) => {
  ...
  return (
    <>
      <div onClick={handleClick} className="hover:underline cursor-pointer">{data || categoriesEditRowMessage.notFound}</div>

      <Sheet open={open} onOpenChange={handleonClose}>
        <SheetContent className="space-y-4">
          <SheetHeader>
            <SheetTitle>{categoriesEditRowMessage.editName}{data}</SheetTitle>
            <SheetDescription>{categoriesEditRowMessage.editExistingName} {data}.</SheetDescription>
          </SheetHeader>
          <LabelForm
          data={data}
          id={id}
