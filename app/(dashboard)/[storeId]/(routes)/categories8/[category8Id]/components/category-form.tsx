...
import { Category8,Billboard } from "@prisma/client"
...
export const CategoryForm: React.FC<CategoryFormProps> = ({
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/categories8/${params.category8Id}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/categories8`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/categories8`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/categories8/${params.category8Id}`);
      router.refresh();
      router.push(`/${params.storeId}/categories8`);
