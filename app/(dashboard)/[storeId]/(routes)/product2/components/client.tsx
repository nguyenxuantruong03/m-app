...
import { getEnterNameTranslation, getProductClient, translateProductsClientPlasticPipe } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientPlasticPipeMessage = translateProductsClientPlasticPipe(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product2`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientPlasticPipeMessage.productPlasticPipe} (${data.length})`}
          description={productClientPlasticPipeMessage.manageProductPlasticPipe}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product2/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product2Id" entityName="product2" />
    </>
  );
};
