...
import { Category9, Product9, Image9, Size, Color, Specifications,Salientfeatures } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/product9/${params.product9Id}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/product9`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/product9/${params.product9Id}`);
      router.refresh();
      router.push(`/${params.storeId}/product9`);
