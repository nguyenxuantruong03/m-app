...
import { getEnterNameTranslation, getProductClient,translateProductsClientElectricalWire } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientElectricalWireMessage = translateProductsClientElectricalWire(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product3`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientElectricalWireMessage.productElectricalWire} (${data.length})`}
          description={productClientElectricalWireMessage.manageProductElectricalWire}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product3/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product3Id" entityName="product3" />
    </>
  );
};
