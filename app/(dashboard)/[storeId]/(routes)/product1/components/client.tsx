...
import { getEnterNameTranslation, getProductClient, translateProductsClientFan } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientFanMessage = translateProductsClientFan(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product1`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientFanMessage.productFan} (${data.length})`}
          description={productClientFanMessage.manageProductFan}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product1/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product1Id" entityName="product1" />
    </>
  );
};
