...
import { Category, Product, Image, Size, Color, Specifications,Salientfeatures, Ipad, ImageIpad, Tivi, ImageTivi } from "@prisma/client"
...
const formSchema = z.object({
  ...
  imagestivi: z.object({url: z.string()}).array(),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const title = initialData ? 'Edit tivi' : 'Create tivi';
  const description = initialData ? 'Edit a tivi.' : 'Add a new tivi';
  const toastMessage = initialData ? 'Tivi updated.' : 'Tivi created.';
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
     ...
     /* Khối mã chịu trách nhiệm thực hiện yêu cầu HTTP để cập nhật bảng quảng cáo hiện có
      ...
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/tivi/${params.tiviId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/tivi`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/tivi`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/tivi/${params.tiviId}`);
      router.refresh();
      router.push(`/${params.storeId}/tivi`);
      toast.success('Tivi deleted.');
    } catch (error: any) {
      toast.error('Make sure you removed all tivi using this product first.');
    } finally {
      setLoading(false);
      setOpen(false);
    }
