...
import { getEnterNameTranslation, getProductClient, translateProductsClientLightBulb } from "@/translate/translate-dashboard";
...
const ProductClient: React.FC<ProductClientProps> = ({ data }) => {
  ...
  const productClientLightBulbMessage = translateProductsClientLightBulb(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/product10`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${productClientLightBulbMessage.productLightBulb} (${data.length})`}
          description={productClientLightBulbMessage.manageProductLightBulb}
        />
        <div className="flex space-x-3">
          ...
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/product10/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="product10Id" entityName="product10" />
    </>
  );
};
