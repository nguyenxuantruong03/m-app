...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories3`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      ...
      <Heading
          title={`${categoriesClientPlasticMessage.categoryElectricalWire} (${data.length})`}
          description={categoriesClientPlasticMessage.manageCategoryElectricalWire}
        />
        <div className=" flex space-x-3">
        <Downloadfile data={data} filename="categories3" languageToUse={languageToUse}/>
          <Button
      ...
      <Separator />
      <ApiList entityIdName="category3Id" entityName="categories3" />
    </>
  );
};
