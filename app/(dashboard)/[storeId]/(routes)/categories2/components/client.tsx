...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientPlastic } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const showAPIRole = isRole;
   ...
   const categoriesClientPlasticMessage = translateCategoriesClientPlastic(languageToUse)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories2`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${categoriesClientPlasticMessage.categoryPlastic} (${data.length})`}
          description={categoriesClientPlasticMessage.manageCategoryPlastic}
        />
        <div className=" flex space-x-3">
          <DownloadFile data={data} filename="categories2" languageToUse={languageToUse}/>
          <Button
