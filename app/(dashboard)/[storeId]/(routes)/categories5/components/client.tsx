...
import { getCategoriesClient, getEnterNameTranslation, translateCategoriesClientLock } from "@/translate/translate-dashboard";
...
const CategoriesClient: React.FC<CategoriesClientProps> = ({ data }) => {
  ...
  const categoriesClientLockMessage = translateCategoriesClientLock(languageToUse);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/${params.storeId}/categories5`, {
        data: { ids },
      });
      setLoading(false);
      setOpen(false);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${categoriesClientLockMessage.categoryLock} (${data.length})`}
          description={categoriesClientLockMessage.manageCategoryLock}
        />
        <div className=" flex space-x-3">
          <Downloadfile
            data={data}
            filename="categories5"
            languageToUse={languageToUse}
          />
          <Button
            onClick={() => router.push(`/${params.storeId}/categories5/new`)}
          >
      ...
      <Separator />
      <ApiList entityIdName="category5Id" entityName="categories5" />
    </>
  );
};
