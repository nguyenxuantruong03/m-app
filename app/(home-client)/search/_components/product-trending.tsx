...
export const ProductTrending = ({ data }: ResultCardProps) => {
      ...
      <Link href={`/${route}/${data.name}`}>
        <div className="w-full h-full gap-x-4">
          ...
          <Currency value={priceSale} valueold={priceold} />
            <p className="font-bold text-lf cursor-pointer hover:text-blue-500 trancate max-w-[8rem] md:max-w-[15rem] overflow-hidden text-ellipsis whitespace-nowrap">
              {data.heading}
            </p>
            <p className="trancate max-w-[8rem] md:max-w-[15rem] overflow-hidden text-ellipsis whitespace-nowrap">
              {data.description}{" "}
            </p>
          </div>
        </div>
