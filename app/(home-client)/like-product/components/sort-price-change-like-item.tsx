...
const PriceRange: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const maxPriceInItems = Math.max(
      ...like.items.map(
        (item) => item.productdetail.price1 * ((100 - item.productdetail.percentpromotion1) / 100) + 1000000
      )
    );
  ...
  const handleRangeInputChange = (
    ...
    const filteredItems = like.items.filter(
      (item) =>
        item.productdetail.price1 * ((100 - item.productdetail.percentpromotion1) / 100) >= minPrice &&
        item.productdetail.price1 * ((100 - item.productdetail.percentpromotion1) / 100) <= maxPrice
    );
