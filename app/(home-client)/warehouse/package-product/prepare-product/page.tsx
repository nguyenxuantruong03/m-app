...
const PrepareProduct = () => {
  ...
  const prepareMessage = getPrepareMessages(languageToUse);

  useEffect(() => {
    if (loading ) {
      document.title = prepareMessage.loading;
    } else {
      document.title = prepareMessage.preparing;
    }
  }, [loading]);
  ...
  const matchId = data.filter((order: Order) => order.status === "Soan_hang");
                ...
                return (
                  <>
                    <div
                      ...
                      onClick={() =>
                        router.push(
                          `/warehouse/package-product/prepare-product/${order.id}`
                        )
                      }
                    >
                      {openReturnProduct && (
