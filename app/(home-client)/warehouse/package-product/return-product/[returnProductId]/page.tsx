...
const WareHouseDetail = ({
  ...
  useEffect(() => {
    if (data.length > 0) {
      const order = data.find((item) => item.id === params.returnProductId);
      if (order) {
  ...
  const matchId = data.filter(
    (item: Order) => item.id === params.returnProductId
  );
