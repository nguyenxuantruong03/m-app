...
const DeliveredProduct = () => {
  ...
  const deliveredMessage = getDeliveredMessages(languageToUse);

  useEffect(() => {
    if (loading ) {
      document.title = deliveredMessage.loading;
    } else {
      document.title = deliveredMessage.delivered;
    }
  }, [loading]);
  ...
  const matchId = data.filter((order: Order) => order.status === "Da_giao");
                ...
                return (
                  <>
                    <div
                      ...
                      onClick={() =>
                        router.push(
                          `/warehouse/package-product/delivered-product/${order.id}`
                        )
                      }
                    >
                      {openReturnProduct && (
