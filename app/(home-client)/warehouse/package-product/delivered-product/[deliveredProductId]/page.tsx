...
import LocationDeliveredProduct from "@/components/(client)/location-product/location-delivered";
import getCart from "@/actions/client/cart";

const WareHouseDetail = ({
  params,
}: {
  params: { deliveredProductId: string };
}) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      const order = data.find((item) => item.id === params.deliveredProductId);
      if (order) {
  ...
  }, [data, params.deliveredProductId]);
  ...
  const matchId = data.filter(
    (item: Order) => item.id === params.deliveredProductId
  );
  ...
  return (
    <div>
      ...
      {matchId.map((order: Order) => (
        ...
        >
           {/* Modal Map Location giao đơn hàng */}
           {isModalOpen && (
            <div
            ...
            >
              ...
              >
                <LocationDeliveredProduct
                  locationLat={order.locationLatEnd}
                  locationLng={order.locationLngEnd}
                />
                <button
                  onClick={closeModal}
