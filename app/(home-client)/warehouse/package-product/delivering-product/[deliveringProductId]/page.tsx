...
const WareHouseDetail = ({
  ...
  params: { deliveringProductId: string };
}) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      const order = data.find((item) => item.id === params.deliveringProductId);
      if (order) {
  ...
  }, [data, params.deliveringProductId]);
  ...
  const matchId = data.filter(
    (item: Order) => item.id === params.deliveringProductId
  );
