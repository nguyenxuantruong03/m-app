...
export async function GET(
  req: Request,
  { params }: { params: { product3Id: string } }
) {
  try {
    if (!params.product3Id) {
...
export async function DELETE(
  req: Request,
  { params }: { params: { product3Id: string; storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.product3.delete({
      where: {
        id: params.product3Id,
      },
    });
...
export async function PATCH(
  ...
  try {
    ...
    if (!imagesalientfeaturesproduct3 || !imagesalientfeaturesproduct3.length) {
      return new NextResponse("Imagesalientfeatures product3 is required", {
        status: 400,
      });
    }
    ...
    await prismadb.product3.update({
      where: {
        id: params.product3Id,
      },
      data: {
    ...
    const product = await prismadb.product3.update({
      where: {
        id: params.product3Id,
      },
      data: {
        ...
        imagesalientfeaturesproduct3: {
          createMany: {
            data: [
              ...imagesalientfeaturesproduct3.map(
