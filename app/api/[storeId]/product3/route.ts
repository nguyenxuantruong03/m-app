...
export async function POST(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productPostMessage.internalError}3` }),
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    const productType = ProductType.PRODUCT3;
    const productType6 = ProductType.PRODUCT6;
    if (!params.storeId) {
      return new NextResponse(
    ...
    // Sort by productType alternating between productType and productType6
    const alternatingProducts = [];
    let productTypeQueue = products.filter(p => p && p.productType === productType);  // Explicit check for undefined
    let productType7Queue = products.filter(p => p && p.productType === productType6);  // Explicit check for undefined
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productGetMessage.internalError}3` }),
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        ...
        type: "DELETEMANY-DÂYĐIỆN-PRODUCT",
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productDeleteMessage.internalError}3` }),
      { status: 500 }
    );
  }
}
