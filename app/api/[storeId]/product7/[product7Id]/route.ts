...
export async function GET(
  req: Request,
  { params }: { params: { product7Id: string } }
) {
  try {
    ...
    const product = await prismadb.product7.findUnique({
      where: {
        id: params.product7Id,
      },
      include: {
        images: true,
        imagesalientfeaturesproduct7: true,
        category: true,
        size: true,
        color: true,
      },
...
export async function DELETE(
  req: Request,
  { params }: { params: { product7Id: string; storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.product7.delete({
      where: {
        id: params.product7Id,
      },
    });
...
export async function PATCH(
  ...
  try {
    ...
    if (!imagesalientfeaturesproduct7 || !imagesalientfeaturesproduct7.length) {
      return new NextResponse("Imagesalientfeatures product7 is required", {
        status: 400,
      });
    }
    ...
    await prismadb.product7.update({
      where: {
        id: params.product7Id,
      },
      data: {
    ...
    const product = await prismadb.product7.update({
      where: {
        id: params.product7Id,
      },
      data: {
        ...
        imagesalientfeaturesproduct7: {
          createMany: {
            data: [
              ...imagesalientfeaturesproduct7.map(
