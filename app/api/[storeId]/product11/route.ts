  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        storeId: params.storeId,
        newChange: changes,
        type: "CREATEĐỒTHƯỜNGDÙNG-PRODUCT",
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productPostMessage.internalError}11` }),
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
   ...
   const totalPages = limit !== undefined ? Math.ceil(totalProducts / limit) : undefined;
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productGetMessage.internalError}11` }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productDeleteMessage.internalError}11` }),
      { status: 500 }
    );
  }
}
