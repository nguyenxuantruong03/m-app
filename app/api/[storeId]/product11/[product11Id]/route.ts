...
export async function GET(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdGetMessage.internalError}11` }),
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        ...
        delete: changes,
        type: "DELETEĐỒTHƯỜNGDÙNG-PRODUCT",
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdDeleteMessage.internalError}11`}),
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        ...
        type: "UPDATEĐỒTHƯỜNGDÙNG-PRODUCT",
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdPatchMessage.internalError}11` }),
      { status: 500 }
    );
  }
}
