...
export async function POST(
  ...
  try {
    ...
    if (!imagesalientfeatures || !imagesalientfeatures.length) {
      return new NextResponse("Imagesalientfeatures Product4 is required", {
        status: 400,
      });
    }
    ...
    const product = await prismadb.product4.create({
      data: {
        name,
        heading,
        description,
...
export async function GET(
  ...
  try {
    ...
    const product = await prismadb.product4.findMany({
      where: {
        storeId: params.storeId,
        categoryId,
        colorId,
