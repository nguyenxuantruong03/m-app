...
export async function GET(
  req: Request,
  { params }: { params: { product5Id: string } }
) {
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdGetMessage.internalError}5` }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdDeleteMessage.internalError}5` }),
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    const existingProductWithSameHeading = await prismadb.product.findFirst({
      where: {
        ...
        storeId: params.storeId,
        NOT: {
          id: params.product5Id, // Loại trừ sản phẩm hiện tại
        },
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productIdPatchMessage.internalError}5` }),
      { status: 500 }
    );
  }
}
