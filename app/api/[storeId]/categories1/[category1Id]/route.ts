  ...
  try {
    if (!params.category1Id) {
      return new NextResponse(
        JSON.stringify({ error: "Category1 id is required!" }),
        { status: 400 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal error delete categories1." }),
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal error patch categories1." }),
      { status: 500 }
    );
  }
}
