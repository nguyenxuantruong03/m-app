...
export async function POST(
  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        storeId: params.storeId,
        type: "CREATEVẬTLIỆUNHÀTẮM-CATEGORY",
        newChange: changes,
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${categoriesPostMessage.name6} 9` }),
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${categoriesGetMessage.name2} 9` }),
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    await prismadb.system.create({
      data: {
        ...
        type: "DELETEMANYVẬTLIỆUNHÀTẮM-CATEGORY",
        user: user?.email || "",
      },
    });
    ...
    return new NextResponse(
      JSON.stringify({ error: `${categoriesDeleteMessage.name6} 9` }),
      { status: 500 }
    );
  }
}
