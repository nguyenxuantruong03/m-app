...
export async function GET(
  req: Request,
  { params }: { params: { product10Id: string } }
) {
  const productType = ProductType.PRODUCT10;
  try {
    if (!params.product10Id) {
      return new NextResponse(
        JSON.stringify({ error: "Product10 id is required!" }),
        { status: 400 }
      );
    }
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal error get product10." }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  req: Request,
  { params }: { params: { product10Id: string; storeId: string } }
) {
  const productType = ProductType.PRODUCT10;
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal error delete product10." }),
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal error patch product10." }),
      { status: 500 }
    );
  }
}
