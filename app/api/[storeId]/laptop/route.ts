...
export async function POST(
  ...
  try {
    ...
    if (!imagesalientfeatureslaptop || !imagesalientfeatureslaptop.length) {
      return new NextResponse("ImagesalientfeaturesLaptop is required", {
        status: 400,
      });
    }
    ...
    const laptop = await prismadb.laptop.create({
      data: {
        ...
        imagesalientfeatureslaptop: {
          createMany: {
            data: [
              ...imagesalientfeatureslaptop.map(
...
export async function GET(
  ...
  try {
    ...
    const categorylaptopId = searchParams.get("categorylaptopId") || undefined;
