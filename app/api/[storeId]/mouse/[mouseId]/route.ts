...
export async function GET(
  req: Request,
  { params }: { params: { mouseId: string } }
) {
  try {
    if (!params.mouseId) {
      return new NextResponse("mouse id is required", { status: 400 });
    }

    const mouses = await prismadb.mouse.findUnique({
      where: {
        id: params.mouseId,
      },
      include: {
...
export async function DELETE(
  req: Request,
  { params }: { params: { mouseId: string; storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.mouse.delete({
      where: {
        id: params.mouseId,
      },
    });
    ...
    console.log("[mouse_DELETE]", error);
    return new NextResponse("Internal error", { status: 500 });
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const product = await prismadb.mouse.update({
      where: {
        id: params.mouseId,
      },
      data: {
