...
export async function POST(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productPostMessage.internalError}6` }),
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    const productType = ProductType.PRODUCT6;
    const productType1 = ProductType.PRODUCT1;
    ...
    // Sort by productType alternating between productType and productType1
    const alternatingProducts = [];
    let productTypeQueue = products.filter(p => p && p.productType === productType);  // Explicit check for undefined
    let productType7Queue = products.filter(p => p && p.productType === productType1);  // Explicit check for undefined
   ...
   const totalPages = limit !== undefined ? Math.ceil(totalProducts / limit) : undefined;
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productGetMessage.internalError}6` }),
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: `${productDeleteMessage.internalError}6` }),
      { status: 500 }
    );
  }
}
