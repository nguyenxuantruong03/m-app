...
export async function GET(
  req: Request,
  { params }: { params: { tiviId: string } }
) {
  try {
    if (!params.tiviId) {
      return new NextResponse("Tivi id is required", { status: 400 });
    }

    const ipads = await prismadb.tivi.findUnique({
      where: {
        id: params.tiviId,
      },
      include: {
...
export async function DELETE(
  req: Request,
  { params }: { params: { tiviId: string; storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.tivi.delete({
      where: {
        id: params.tiviId,
      },
    });
...
export async function PATCH(
  ...
  try {
    ...
    const product = await prismadb.tivi.update({
      where: {
        id: params.tiviId,
      },
      data: {
