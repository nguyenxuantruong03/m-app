...
export async function POST(
  ...
  try {
    ...
    if (!imagesalientfeaturesproduct1 || !imagesalientfeaturesproduct1.length) {
    ...
    const product = await prismadb.product1.create({
      data: {
        name,
        heading,
        description,
        ...
        imagesalientfeaturesproduct1: {
          createMany: {
            data: [
              ...imagesalientfeaturesproduct1.map(
...
export async function GET(
  ...
  try {
    ...
    const product = await prismadb.product1.findMany({
      where: {
        storeId: params.storeId,
        categoryId,
        colorId,
