...
const Game2048 = () => {
  ...
  useEffect(() => {
    ...
    if (gameOver) {
      ...
      const newTotalCoins = Math.floor(score / 100);

      // Save the new totalCoins to the database
      updateTotalCoinsAndSave(newTotalCoins);
      audio.play();
      setMessage(
        `Game OVER! Tổng xu bạn nhận: ${Math.floor(
          score / 100
        )} xu. Tổng điểm ${score}`
      );
    }
  ...
  return (
    <>
      <div id="game-container" ref={gameContainerRef}>
        ...
        <div className="my-4  text-lg font-bold flex ml-[580px] text-slate-900 dark:text-slate-200">
          Tổng điểm: {score}
        </div>
