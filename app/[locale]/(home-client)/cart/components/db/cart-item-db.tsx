...
const CartItemDatabase: React.FC<CartItemProps> = ({
  ...
  const handleQuantityChange = async (
    ...
    if (value > 0 && value <= maxQuantity) {
      ...
      toast.error(t("product.remainingProduct", { maxQuantity }));
    }
  };
  ...
  const incrementQuantity = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (quantity) {
      ...
      if (quantity < maxQuantity) {
        ...
        toast.error(toast.error(t("product.remainingProduct", { maxQuantity })));
      }
    }
  };
  ...
  return (
    ...
    >
          ...
          <span>
            {quantityInventory ? (
              ...
              <div className="p-1 rounded-md bg-red-500 text-white text-center">
                <p className="break-words">{t("product.notEnoughStock")}</p>
              </div>
            )}
          </span>
        ) : (
      ...
      <div className="relative ml-4 flex flex-1 flex-col justify-between sm:ml-6">
        ...
        {isOutOfStock && (
          <div className="text-red-500">{t('product.remainingProduct', { maxQuantity })}</div>
        )}

        <div className="mt-1 text-sm text-gray-500">
          {t("product.warrantyPrice")} {data.product.heading}:
          {data.warranty ? (
            <span>
