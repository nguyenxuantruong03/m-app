...
const SumaryDb: React.FC<SumaryProps> = ({
  ...
  const onRemoveAll = async () => {
    try {
      ...
      toast.success(t("cart.cartItemsDeleted"));
    } catch (error) {
  ...
  return (
    <>
      <AlertModal
        title={t("product.cofirmDeleteProduct")}
        message={t("action.actionIrreversible")}
            ...
            <h2 className="text-base md:text-lg font-medium text-gray-900">
              ...
              {cartdb.selectedItems.length <= 0 ? (
                ...
                <span className="text-gray-400 text-xs md:text-sm">
                  ({t("product.selectItems", { selectedItemsLength: cartdb.selectedItems.length })})
                </span>
              )}
            </h2>
          </div>
          ...
          >
            <div className="container w-full md:w-3/4 lg:w-2/3 hover:w-[360px] md:hover:w-[700px] lg:hover:w-[1100px]">
              ...
              <div className="right-side">
                <div className="new">{t("cart.payMent")}</div>
