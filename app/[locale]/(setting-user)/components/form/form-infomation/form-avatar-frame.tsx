...
const FormAvatarandFrame = ({languageToUse}:FormAvatarandFrameProps) => {
  ...
  const onSubmit = (values: z.infer<typeof SettingSchema>) => {
    ...
    if (!values.frame) {
      setError(t("formInfo.chooseFrameForAccount"));
      return;
    }

    if (values.frame === user?.frameAvatar) {
      setError(t("formInfo.frameAlreadySelected"));
      return;
    }
  ...
  return (
    <Form {...form}>
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="space-y-4">
          <FormLabel>{t("formInfo.chooseAvatarFrame")}</FormLabel>
        ...
        {(role === UserRole.ADMIN || role === UserRole.STAFF) && (
          <div className="space-y-4">
            <FormLabel>{t("formInfo.chooseVIPAvatarFrame")}</FormLabel>
            <div className="grid grid-cols-6 gap-2 h-32 overflow-y-auto">
