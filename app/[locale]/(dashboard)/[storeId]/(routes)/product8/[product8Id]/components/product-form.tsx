...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if (initialData) {
        promise = axios.patch(
          `/api/${params.storeId}/product8/${params.productId}`,
          data
        );
      } else {
        promise = axios.post(`/api/${params.storeId}/product8`, data);
      }
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/product8/${params.productId}`);
      router.refresh();
      router.push(`/${params.storeId}/product8`);
