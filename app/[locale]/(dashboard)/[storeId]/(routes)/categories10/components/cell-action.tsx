...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = (id: string) => {
    ...
    toast.success(t("action.copyId"));
  };

  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/categories10/${data.id}`);
      router.refresh();
      toast.success(t("category.cellaction.deleted"));
    } catch (error: unknown) {
      if (
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/categories10/${data.id}`)
            }
          >
