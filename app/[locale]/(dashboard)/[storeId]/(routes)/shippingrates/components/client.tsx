...
const ShippingRatesClient: React.FC<ShippingRatesClientProps> = ({ data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setLoading(false);
      setOpen(false);
      toast.success(t("shippingrate.client.shippingRateDeletedSuccessfully"));
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${t("shippingrate.shippingRate")} (${data.length})`}
          description={t("shippingrate.client.manageShippingRates")}
        />
      ...
      <Separator />
      <DataTable
        placeholder={t("shippingrate.client.enterName")}
        searchKey="name"
        columns={columns}
        data={data}
        onSelect={(rows) => {
      ...
      />
      {showAPIRole && (
        <Heading title={t("action.api")} description={t("shippingrate.client.apiCallsForShippingRates")} />
      )}
      <Separator />
