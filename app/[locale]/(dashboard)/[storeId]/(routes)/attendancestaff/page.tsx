...
export default function Home() {
  ...
  const [events, setEvents] = useState([
    { title: t("attendance.birthday"), id: "1" },
    { title: t("attendance.overtime"), id: "2" },
    { title: t("attendance.shiftChange"), id: "3" },
    { title: t("attendance.dayOff"), id: "4" },
    { title: t("attendance.busy"), id: "5" },
    { title: t("attendance.other"), id: "6" },
  ]);
  ...
  useEffect(() => {
    if (userId) {
      if (!userId.timestartwork) {
        toast.error(t("attendance.noWorkTime"));
        return;
      }
      const now = new Date();
  ...
  const handleCheckAttendanceStart = async () => {
    if (selectedDate) {
      ...
      if (!isToday) {
        ...
        toast.error(t("attendance.cannotCheckOutForAnotherDay"));
        setIsCheckingAttendanceStart(false);
      ...
      if (dateWorkAttendance && !dateWorkAttendance.includes(dayName)) {
        toast.error(t("attendance.notYourWorkday"));
        setIsCheckingAttendanceStart(false);
        setIsCheckingAttendanceEnd(false);
        setIsAddingEvent(false);
        setIsDeleting(false);
      ...
      try {
        ...
        if (userId && userId.timestartwork) {
          ...
          if (currentTime.getTime() < currentDate.getTime()) {
            toast.error(
              `${t("attendance.checkInNotYetTime")}:${userId?.timestartwork}!`
            );
          } else {
            ...
            if (existingEventToday || existingEventTomorrow) {
              ...
              toast.error(t("attendance.alreadyCheckedIn"));
              setIsCheckingAttendanceEnd(false);
              setIsCheckingAttendanceStart(false);
              setIsAddingEvent(false);
              setIsDeleting(false);
        ...
        if (
          ...
          // Hiển thị thông báo lỗi mặc định cho người dùng
          toast.error(t("attendance.checkOutNotYetTime"));
        }
  ...
  const handleCheckAttendanceEnd = async () => {
    if (selectedDate) {
      ...
      try {
        if (isEnd === true) {
          toast.error(t("attendance.alreadyCheckedOut"));
          setIsCheckingAttendanceEnd(false);
          setIsCheckingAttendanceStart(false);
          setIsAddingEvent(false);
          setIsDeleting(false);
    ...
    if (data.date) {
      ...
      if (!eventExists) {
        ...
        if (eventsCountForCurrentDate < 3) {
          ...
          axios
                  ...
                  <div className="flex-1 w-0 p-4">
                    <div className="flex items-start">
                      ...
                      <div className="ml-3 flex-1">
                        <p className="text-green-500 font-bold flex">
                          <Check className="w-5 h-5 rounded-full bg-green-500 text-white mx-1" />
                          t("attendance.eventAddedSuccess")
                        </p>
                        <p className="mt-1 text-sm text-gray-500">
                          t("attendance.user")
                          <span className="font-bold text-blue-500">
                          ...
                          - <span className="font-bold">{userId?.email}</span>.
                          t("attendance.eventAddedMessage")
                          <span className="font-bold text-red-500">
                            {event.title}
                          </span>{" "}
                          t("attendance.eventAddedAt"):{" "}
                          <span className="font-bold">
                            {response.data?.start
                              ? format(
                                  utcToZonedTime(
            ...
            .catch((error) => {
              ...
              if (
                ...
                // Hiển thị thông báo lỗi mặc định cho người dùng
                toast.error(t("attendance.eventAddError"));
              }
              setIsCheckingAttendanceEnd(false);
              setIsCheckingAttendanceStart(false);
              setIsAddingEvent(false);
          ...
          toast.error(t("attendance.eventLimit",{eventTitle: event.title}));
          setIsCheckingAttendanceEnd(false);
          setIsCheckingAttendanceStart(false);
          setIsAddingEvent(false);
          setIsDeleting(false);
  ...
  function handleDelete() {
    if (idToDelete !== null) {
      ...
      if (isEventFound) {
        ...
        toast.error(t("attendance.cannotDeleteEvent"));
        setIsDeleting(false);
        return;
      }
      ...
      axios
              ...
              <div className="flex-1 w-0 p-4">
                <div className="flex items-start">
                  ...
                  <div className="ml-3 flex-1">
                    <p className="text-green-500 font-bold flex">
                      <Check className="w-5 h-5 rounded-full bg-green-500 text-white mx-1" />
                      t("attendance.eventDeletedSuccess")
                    </p>
                    <p className="mt-1 text-sm text-gray-500">
                      t("attendance.user")
                      <span className="font-bold text-blue-500">
                      ...
                      - <span className="font-bold">{userId?.email}</span>.{" "}
                      t("attendance.eventDeletedMessage")
                      {response.data.title} t("attendance.eventDeletedAt"):{" "}
        ...
        .catch((error) => {
          ...
          if (
            ...
            // Hiển thị thông báo lỗi mặc định cho người dùng
            toast.error(t("attendance.eventDeleteError"));
          }
        })
        .finally(() => {
      ...
      if (idToDelete !== null) {
        toast.error(t("attendance.invalidIdToDelete",{idToDelete: idToDelete}));
      }
    }
  }
