...
const Camera: React.FC<CameraPorps> = ({
  ...
  const getVideo = () => {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          if (video) {
            ...
            toast.error(t("attendance.camera.cameraNotFound"));
          }
        })
        .catch((err) => {
      ...
      toast.error(t("attendance.camera.browserNotSupported"));
    }
  };
  ...
  const takePhoto = () => {
    ...
    if (photo) {
      ...
      if (ctx) {
        ...
        toast.error(t("attendance.camera.cannotLoadImage2D"));
      }
    } else {
      toast.error(t("attendance.camera.imageNotFound"));
    }
  };
        ...
        if (isCheckAttendanceImages[0] === false) {
          ...
          await axios
            ...
            .then((response) => {
              ...
              toast.success(t("attendance.camera.imageUploadedSuccess"));
            })
            .catch((error) => {
              setIsLoadingData(false);
              toast.error(t("toastError.somethingWentWrong"));
          ...
          toast.error(t("attendance.camera.alreadyCheckedInToday"));
        }
      } else {
        toast.error(t("attendance.camera.invalidQrCode"));
      }
    } else {
      toast.error(t("attendance.camera.imageNotFound"));
    }
        ...
        <div className="relative">
          ...
          {showMessage && (
            ...
            <p className=" absolute top-2 left-1/2  transform -translate-x-1/2 border-yellow-400 border-2 flex items-center p-3">
              <TriangleAlert className="w-4 h-4 text-yellow-400 mr-3" /> {t("attendance.alreadyCheckedIn")}
            </p>
          )}
          {showMessageNFC && (
            // Sử dụng showMessage để kiểm soát việc hiển thị thông báo
            <p className=" absolute top-2 left-1/2  transform -translate-x-1/2 border-yellow-400 border-2 flex items-center p-3">
              <TriangleAlert className="w-4 h-4 text-yellow-400 mr-3" /> {t("attendance.camera.checkedInByNfc")}
            </p>
          )}
        </div>
      )}
              ...
              >
                {qrCodeValid ? (
                  <>
                    ...
                    {t("attendance.camera.qrCodeCorrect")}
                  </>
                ) : (
                  <>
                    <X className="w-4 h-4 text-red-500 mr-3" />
                    {t("attendance.camera.qrCodeIncorrect")}
                  </>
                )}
              </p>
            )}
