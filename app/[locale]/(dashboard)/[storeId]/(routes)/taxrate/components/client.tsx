...
const TaxRateClient: React.FC<TaxRateClientProps> = ({ data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setLoading(false);
      setOpen(false);
      toast.success(t("taxrate.client.taxRateDeletedSuccessfully"));
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${t("taxrate.tax")} (${data.length})`}
          description={t("taxrate.client.taxManagement")}
        />
      ...
      <Separator />
      <DataTable
        placeholder={t("taxrate.client.enterName")}
        searchKey="name"
        columns={columns}
        data={data}
        onSelect={(rows) => {
      ...
      />
      {showAPIRole && (
        <Heading title={t('action.api')} description={t("taxrate.client.apiCallsForTaxRate")} />
      )}
      <Separator />
