...
const SizeClient: React.FC<SizeClientProps> = ({ data }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setLoading(false);
      setOpen(false);
      toast.success(t("size.client.sizeDeletedSuccess"));
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Heading
          title={`${t("size.size")} (${data.length})`}
          description={t("size.client.manageSize")}
        />
      ...
      <Separator />
      <DataTable
        placeholder={t("size.client.enterName")}
        searchKey="name"
        columns={columns}
        data={data}
        onSelect={(rows) => {
        ...
        open={open}
      />
      {showAPIRole && <Heading title={t("action.api")} description={t("size.client.apiCallsForSize")}/>}
      <Separator />
